...
double CalcOffset(const char *rootFileName, const char *histogramName, Int_t np = 10, Int_t NpeaksToExtrapolate = 20, bool PrintVerbose = false) {
    Int_t p;
    ...
    TH1 *h = dynamic_cast<TH1*>(outputFile->Get(histogramName));
    TH1 *h_unfiltered = dynamic_cast<TH1*>(outputFile->Get(histogramName));
    ...
    h_thres->SetTitle(histogramName);
    h_thres->SetStats(0);
    ...
    h_unfiltered->Draw("same HIST");

    h_thres->SetLineColor(kBlue);
    h_thres->Draw("same HIST");

    // Store the position of the peaks
    Double_t *xpeaks;
    ...
    if( nfound < NpeaksToExtrapolate ) return 280;
    ...
    // Make an array of all the DAC voltages
    ...
    for (size_t i = 0; i < NpeaksToExtrapolate; i++) {
    ...
    // Perform the fit
    centroidGraph->Fit("polynomialFit", "Q");
    ...
    latex.DrawLatex(0.3, 0.70, Form("y = %.5fx + %.3f", slope, intercept)); 
    ...
    // // Clean up
    delete c1;
    delete polynomialFit;
    delete centroidGraph;
...
void AnalysePulserWalk(const std::string& inputFileName, bool DoCalcOffset = false, bool writeEvenChannels = true) {
    ...
    if (writeEvenChannels) {
        Offset_outfile.open("Offset_EvenCh.cal");
    }
    else {
        Offset_outfile.open("Offset_OddCh.cal");
    }
    ...
    for (int mod = 0; mod < 3; ++mod) {
        ...
        for (int asic = 0; asic < 6; ++asic) {
            ...
            for (int i = 0; i < 128; ++i) {
                ...
                h2 = static_cast<TH2F*>(inputFile->Get(histName.c_str()));
                ...
                projection = h2->ProjectionY(asicNameproj.c_str(), i + 1, i + 1);
                ...
                if ((i % 2 == 0 && writeEvenChannels) || (i % 2 != 0 && !writeEvenChannels)) {
                   ...
                   //projection->Write();
                    ...
                    if (DoCalcOffset) {
                        myOffset = CalcOffset(outputFileName.c_str(), asicNameproj.c_str());
                        Offset_outfile << std::string(asicNameproj) + ".Offset:     " << myOffset << endl;
                        Offset_outfile << std::string(asicNameproj) + ".Gain:     10.0" << endl;
                        Offset_outfile << std::string(asicNameproj) + ".GainQuadr:     0.0" << endl;
                    }
...
void OffsetProjRun() {
    ...
    //AnalysePulserWalk("R197add_EvenHigh.root", true, true);
    AnalysePulserWalk("R198add_OddHigh.root", true, false);
    // AnalysePulserWalk("R199_EvenLow.root", false, true);
    // AnalysePulserWalk("R200_OddLow.root", false, false);
}

void runFit_test(){
    std::vector<double> offsets;
    std::vector<const char*> histNames;  // Vector to store histogram names
    std::ofstream Offset_outfile("Offset_test.cal");
    ...
    offsets.push_back(CalcOffset(fileName, histName1));
    offsets.push_back(CalcOffset(fileName, histName2));

    histNames.push_back(histName1);
    histNames.push_back(histName2);

    // Print or use offsets as needed
    for (size_t i = 0; i < offsets.size(); ++i) {
        Offset_outfile << std::string(histNames[i]) + ".Offset:     " << offsets[i] << endl;
    }
}
