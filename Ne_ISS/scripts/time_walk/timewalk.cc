...
void p_recoil(){
    ...
    infile = new TFile("../../my_data/sorted_hists_11042023/DeadLayer_0p3um/hists_gamma/22Ne_R14_to_R40_notall_sumhists_20230403_140924.root" ); //read in input file
    ...
    for ( int j = 1; j < 2; j++ ){
        ...
        //READ IN HISTOGRAM -----------------------------------------------
            tw_recoil_array[j][k] = (TH2F*)infile->Get( Form( "Timing/tw_recoil_array_mod_%i_row%i", j, k ) );
        // ----------------------------------------------------------------
...
//     TFile *infile = new TFile( "/Volumes/ad/work/iss_data/IS710_22/eventsfiles/R80_events.root" ); //read in file
//     ofstream outfile( "/Volumes/ad/work/iss_data/IS710_22/eventsfiles/pn_timewalk.cal" ); //open outfile
...
//             //take histogram
//             pn_td[j][k] = (TH1F*)infile->Get( Form( "array/module_%i/pn_td_mod%i_row%i", j, j , k ) );
//             pn_td_Ep[j][k] = (TH2F*)infile->Get( Form( "array/module_%i/pn_td_En_mod%i_row%i", j, j , k ) );
//             pn_td_En[j][k] = (TH2F*)infile->Get( Form( "array/module_%i/pn_td_En_mod%i_row%i", j, j , k ) );
