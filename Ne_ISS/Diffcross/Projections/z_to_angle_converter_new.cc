...
void z_to_theta_run() {
    z_to_angle_conveter("reaction_22Ne_R14_to_R45.dat");

    std::vector<DataPoint> z_values = x_val_reader("z_values.dat");

    std::ofstream theta_outfile("theta_values_E1016.dat");
    theta_outfile << "projection, theta_min, theta_max, theta_avg" << endl; 

    double E_x = 1016.926; // Set the desired value of E_x here
