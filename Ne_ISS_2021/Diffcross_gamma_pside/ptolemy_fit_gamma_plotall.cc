...
void ptolemy_fit_1_state(const std::string& ptolemy_file, const std::string& hdtv_file, const std::string& zfile, const std::string& theta_file, const std::string& eff_file, const std::string& old_diffcross_file, double IN, int Ex, int l, TCanvas* globalCanvas, int fitIndex, std::vector<double>& sValues){
    
    globalCanvas->cd(fitIndex + 1);
    ...
    EXP_data->SetTitle(0);
    EXP_data->GetXaxis()->SetLabelSize(0.07);
    EXP_data->GetXaxis()->SetTitleSize(0.05);
    EXP_data->GetYaxis()->SetLabelSize(0.06);
    EXP_data->GetYaxis()->SetTitleSize(0.04);
    ...
    // Get the 'S' value and save it
    double sValue = myFunc->GetParameter(0);
    sValues.push_back(sValue);

    std::string Save_img = Form("temp/E%d_l%d_Absdiffcross_Elum_recoil_temp.png", Ex, l);
    globalCanvas->SaveAs(Save_img.c_str());
...
void run_fits_plotall() {
    // IN normalisation from ELUM analysis
    double IN = 466.9914851885867;

    std::vector<double> sValues;

    TCanvas *globalCanvas = new TCanvas("globalCanvas", "Global Fits", 1600, 1400);
    globalCanvas->Divide(3, 2);  // Divide the canvas into 5 vertical pads

    // Loop over the different fits
    for (int fitIndex = 0; fitIndex < 5; ++fitIndex) {
        double energy, angularMomentum;
        std::string ptolemyFile, hdtvFile, zFile, thetaFile, effFile, oldDiffCrossFile;
        ...
        switch (fitIndex) {
            case 0:
                energy = 0;
                angularMomentum = 2;
                ptolemyFile = "ptolemy_clean/22Ne_2021_E_0.out-clean";
                hdtvFile = "hdtv_fits/hdtv_E0-clean.txt";
                thetaFile = "theta_z/theta_values_E0.dat";
                oldDiffCrossFile = "Ne_1967_diffcross/abso_0.txt";
                break;
            case 1:
                energy = 1016;
                angularMomentum = 0;
                ptolemyFile = "ptolemy_clean/22Ne_2021_E_1016.out-clean";
                hdtvFile = "hdtv_fits/hdtv_E1016-clean.txt";
                thetaFile = "theta_z/theta_values_E1016.dat";
                oldDiffCrossFile = "Ne_1967_diffcross/abso_1016.txt";
                break;
            case 2:
                energy = 2315;
                angularMomentum = 2;
                ptolemyFile = "ptolemy_clean/22Ne_2021_E_2315.out-clean";
                hdtvFile = "hdtv_fits/hdtv_E2315-clean.txt";
                thetaFile = "theta_z/theta_values_E2315.dat";
                oldDiffCrossFile = "Ne_1967_diffcross/abso_2315.txt";
                break;
            case 3:
                energy = 3220;
                angularMomentum = 1;
                ptolemyFile = "ptolemy_clean/22Ne_2021_E_3220.out-clean";
                hdtvFile = "hdtv_fits/hdtv_E3220-clean.txt";
                thetaFile = "theta_z/theta_values_E3220.dat";
                oldDiffCrossFile = "Ne_1967_diffcross/abso_3220.txt";
                break;
            case 4:
                energy = 3431;
                angularMomentum = 2;
                ptolemyFile = "ptolemy_clean/22Ne_2021_E_3431.out-clean";
                hdtvFile = "hdtv_fits/hdtv_E3431-clean.txt";
                thetaFile = "theta_z/theta_values_E3431.dat";
                oldDiffCrossFile = "Ne_1967_diffcross/abso_3431.txt";
                break;
            default:
        ...
        // // Debug output to check parameter values for each case
        // std::cout << "Case " << fitIndex << " Parameters: "
        //       << "Energy = " << energy << ", AngularMomentum = " << angularMomentum
        //       << ", ptolemyFile = " << ptolemyFile << ", hdtvFile = " << hdtvFile
        //       << std::endl;
        
        // Call the fitting function for each set of parameters
        ptolemy_fit_1_state(ptolemyFile, hdtvFile, zFile, thetaFile, effFile, oldDiffCrossFile, IN, energy, angularMomentum, globalCanvas, fitIndex, sValues);

        // Move to the next pad in the global canvas for the next fit
    }

    // Print or use the 'S' values as needed
    for (size_t i = 0; i < sValues.size(); ++i) {
        std::cout << "Fit " << i << " - S value: " << sValues[i] << std::endl;
    }

    // Save the global canvas
    std::string saveGlobalImage = "All_states.png";
    globalCanvas->SaveAs(saveGlobalImage.c_str());
}
