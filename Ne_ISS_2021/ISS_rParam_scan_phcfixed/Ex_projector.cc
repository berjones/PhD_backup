...
int** bin_spacer(int row_bin_low, int row_bin_up, int NUM_proj) {
    ...
    /// print out the low and up bins for each projection
    // for (int i = 0; i < NUM_proj; i++) {
    //     std::cout << "bin low: " << bin_lowup[i][0] << ", bin up: " << bin_lowup[i][1] << std::endl;
    // }
...
void Ex_projector(const std::string& hists_file, std::string module = "sum"){
    ...
    if (module == "sum"){
        ...
        TFile root_outfile(("projections/proj_" + hists_file.substr(hists_file.rfind("/") + 1)).c_str(), "RECREATE");
        for (int i = 0; i < row1_div; i++) {
        ...
        std::ofstream x_outfile("reaction/z_values.dat");
